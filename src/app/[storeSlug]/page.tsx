...
export default function StorePage() {
  ...
  if (store?.store_theme) {
    return (
      <>
        {(() => {
          const tiktokIds = (store.tiktok_pixel_id ?? '').split(',').map((s: string) => s.trim()).filter(Boolean)
          if (!tiktokIds.length) return null
          return (
            <Script id="tiktok-pixel" strategy="afterInteractive">{`
              !function(w,d,t){w.TiktokAnalyticsObject=t;var ttq=w[t]=w[t]||[];ttq.methods=["page","track","identify","instances","debug","on","off","once","ready","alias","group","enableCookie","disableCookie"];ttq.setAndDefer=function(t,e){t[e]=function(){t.push([e].concat(Array.prototype.slice.call(arguments,0)))}};for(var i=0;i<ttq.methods.length;i++)ttq.setAndDefer(ttq,ttq.methods[i]);ttq.instance=function(t){for(var e=ttq._i[t]||[],n=0;n<ttq.methods.length;n++)ttq.setAndDefer(e,ttq.methods[n]);return e};ttq.load=function(e,n){var i="https://analytics.tiktok.com/i18n/pixel/events.js";ttq._i=ttq._i||{};ttq._i[e]=[];ttq._i[e]._u=i;ttq._t=ttq._t||{};ttq._t[e]=+new Date;ttq._o=ttq._o||{};ttq._o[e]=n||{};var o=document.createElement("script");o.type="text/javascript";o.async=!0;o.src=i+"?sdkid="+e+"&lib="+t;var a=document.getElementsByTagName("script")[0];a.parentNode.insertBefore(o,a)};${tiktokIds.map((id: string) => `ttq.load("${id.replace(/"/g, '\\"')}");`).join('')}ttq.page();}(window,document,"ttq");
            `}</Script>
          )
        })()}
  ...
  return (
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} style={{ fontFamily: 'system-ui, -apple-system, Arial, sans-serif', background: '#080808', minHeight: '100vh', color: '#fff', overflowX: 'hidden' }}>
      ...
      {(() => {
        const metaIds = (store.meta_pixel_id ?? '').split(',').map((s: string) => s.trim()).filter(Boolean)
        if (!metaIds.length) return null
        return (
          <>
            <Script id="meta-pixel" strategy="afterInteractive">
              {`
                ...
                ${metaIds.map((id: string) => `fbq('init','${id.replace(/'/g, "\\'")}');`).join('\n                ')}
                fbq('track','PageView');
              `}
            </Script>
            {metaIds.map((id: string) => (
              <noscript key={id}>
                <img height="1" width="1" style={{display:'none'}}
                  src={`https://www.facebook.com/tr?id=${encodeURIComponent(id)}&ev=PageView&noscript=1`} alt="" />
              </noscript>
            ))}
          </>
        )
