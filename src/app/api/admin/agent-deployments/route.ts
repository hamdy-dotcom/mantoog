...
export async function POST(req: NextRequest) {
  ...
  const { campaignId, campaignName, smartPlus, agents, targetCpa, maxSpendNoOrders, alertEmail, currency } =
    await req.json().catch(() => ({}))
  const list: string[] = Array.isArray(agents) ? agents.filter((a: string) => ['guardian', 'reporter'].includes(a)) : []
