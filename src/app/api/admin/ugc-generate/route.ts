...
function three60Prompt(dimensions: string): string {
  const dimLine = dimensions
    ...
    : ''
  return `Create ONE clean e-commerce product reference image on a pure-white seamless background, laid out as a neat evenly-spaced GRID (rows and columns). The TOP rows show THIS exact product (the main unit) from MANY angles — front, back, left side, right side, top, and several three-quarter/rotated views — like a 360-degree turntable. Then, in the remaining cells, show EVERY included accessory, attachment, nozzle, brush, hose, pad, adapter and tool that comes bundled with the product, each laid out clearly and separately. Include ALL parts and accessories that appear anywhere across the reference photos — do NOT omit any of them; the bundle must look complete. Every item is part of the SAME one real product set; keep each item's exact shape, colour, proportions, materials, buttons and details identical to the reference photos, evenly lit with soft studio lighting, crisp focus, subtle soft shadows.${dimLine} No people, no text other than the dimension labels, no background props or clutter — only the product views, its accessories, and the dimension callouts.`
}
...
- SHOW THE BENEFIT CORRECTLY — the product's real function and its result must make physical sense. The surface/subject must IMPROVE, never get worse: a cleaner removes dirt so the surface becomes visibly cleaner (never adds dirt); a fan/cooler blows air; a steamer emits steam that lifts grime away. Never depict the product doing the opposite of its purpose.
