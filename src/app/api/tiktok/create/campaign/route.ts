...
import { createTikTokCampaign } from '@/lib/tiktok/create-ad/service'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!payload?.product?.title || !payload?.targeting?.goal) {
      return NextResponse.json({ error: 'invalid_body' }, { status: 400 })
    }

    const result = await createTikTokCampaign(connection, payload)
