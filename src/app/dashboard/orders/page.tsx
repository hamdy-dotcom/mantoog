...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [ordersData, { data: creditsData }, { count: abCount }] = await Promise.all([
        fetchAll(
        ...
        supabase.from('abandoned_checkouts').select('id', { count: 'exact', head: true }).eq('merchant_id', ctx.user.id).eq('recovered', false),
      ])

      setCreditsRemaining(creditsData?.credits_remaining ?? null)
      setOrders(ordersData)
      setAbandonedCount(abCount ?? 0)
      setLoading(false)
    }
    init()
  }, [])
      ...
      <main className={DASHBOARD_MAIN_CLASS}>
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            <a
              href="/dashboard/missed-orders"
              className={`relative text-xs px-3 py-2 rounded-lg transition-colors flex items-center gap-1.5 ${
                abandonedCount > 0
                  ? 'bg-[#3a2800] hover:bg-[#4a3800] border border-[#fbbf24]/30 text-[#fbbf24]'
                  : 'bg-[#1f2229] hover:bg-[#2a2d35] border border-[#2a2d35] text-[#8b8fa8] hover:text-white'
              }`}
            >
              {lang === 'ar' ? 'طلبات ناقصة' : 'Missed Orders'}
              {abandonedCount > 0 && (
                <span className="min-w-[18px] h-[18px] px-1 rounded-full bg-[#fbbf24] text-black text-[9px] font-bold flex items-center justify-center leading-none">
