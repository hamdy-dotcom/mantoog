...
function ProductCard({ product, k, currency, onClick }: { product: any; k: StoreTheme; currency: string; onClick: () => void }) {
  ...
  // default card (standard, wellness, tech, playful, clinical, artisan)
  return (
...
/* ── HERO VARIANTS ─────────────────────────────────────────────── */
function StoreHero({ k, store, onShop }: { k: StoreTheme; store: any; onShop: () => void }) {
  const name = store.name
  const tagline = store.tagline || (store.language === 'ar' ? 'اكتشف منتجات مميزة بأفضل الأسعار' : 'Discover premium products at the best prices')
...
export default function ThemedStoreFront({ store, products }: { store: any; products: any[] }) {
  ...
  const handleShop = () => {
    if (firstProduct) router.push(`/${params.storeSlug}/${firstProduct.id}`)
  }
  ...
  return (
    <div dir="rtl" style={{ background: k.pageBg, color: k.text, fontFamily: k.font, minHeight: '100vh' }}>
      ...
      {/* Hero */}
      <StoreHero k={k} store={store} onShop={handleShop} />
